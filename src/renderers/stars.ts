...
import { getRangeValue } from "@/utils/index";
...
export const starsRenderer: typeof baseRenderer = (...args) => {
  const [instance, td, row, column, prop, value, cellProperties] = args;
  ...
  td.innerHTML = `<div class="star htCenter">${"★".repeat(
    getRangeValue(value, minAllowedValue, maxAllowedValue),
  )}</div>`;
};
